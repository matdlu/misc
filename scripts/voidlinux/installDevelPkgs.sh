...
for potential_devel_pkg in $potential_pkgs_devel; do
	out="$(xbps-query -Rs "$potential_devel_pkg" | grep "$potential_devel_pkg" | head -n 1 | cut -d' ' -f2)"
	pkgs_devel="$pkgs_devel $out"
	if test -n "$out"; then
		echo $out
	else
		echo NOT FOUND $potential_devel_pkg
	fi
done

xbps-install $pkgs_devel
